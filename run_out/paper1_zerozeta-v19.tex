...
Suppose for contradiction that $\zeta(\rho_0)=0$ with $\rho_0=\beta_0+i\gamma_0$ and $\beta_0\ge 0.6$.
Set $\delta_0:=\beta_0-\tfrac12\ge 0.1$.

\medskip\noindent\emph{Choice of Whitney parameter.}
Let $A:=Z_0\,C_{\rm test}\,\sqrt{2C_{\rm box}(\alpha')}$ be the structural constant from the CR--Green
and energy bounds (depending only on $\alpha'$ and the window~$\psi$; see~\eqref{eq:upper-CRG}).
Choose a fixed
\[
  c\ \in\ \Bigl(0,\;\frac{(11/A)^2\,\log\sqrt{2}}{1}\Bigr),
  \qquad
  L\ :=\ \min\!\Bigl\{\frac{c}{\log\angles{\gamma_0}},\;1\Bigr\}.
\]
Then $L\le 1$ always, and $A\sqrt{L}\le A\sqrt{c/\log\sqrt{2}}<11$.

\medskip\noindent\emph{Step 1 (phase-velocity lower bound).}
By Theorem~\ref{thm:phase-velocity-quant}, the distributional boundary-phase derivative $-w'$ of $\mathcal J_{\rm out}$
is a positive measure that includes the Poisson balayage of every off-critical zero.
In particular, $-w'$ dominates the term
$2\delta_0/(\delta_0^2+(t-\gamma_0)^2)$ coming from $\rho_0$.
Center a window $\psi_{L,\gamma_0}$ at $\gamma_0$.
Since $\psi_{L,\gamma_0}\equiv 1$ on $[\gamma_0-L,\gamma_0+L]$, $L\le 1$, and $\delta_0\ge 0.1$:
\begin{equation}\label{eq:lower-zero}
  \int_{\mathbb R}\psi_{L,\gamma_0}(t)\,(-w'(t))\,dt
  \;\ge\; 4\pi\arctan\!\bigl(L/\delta_0\bigr)
  \;\ge\; \frac{4\pi L}{\delta_0+L}
  \;\ge\; \frac{4\pi L}{1.1}
  \;>\; 11\,L.
\end{equation}

\medskip\noindent\emph{Step 2 (CR--Green upper bound).}
By Proposition~\ref{prop:length-free} applied to the admissible test $\varphi_{L,\gamma_0}=Z_0^{-1}L^{-1}\psi_{L,\gamma_0}$
and then multiplied by $Z_0 L$:
\[
  \int_{\mathbb R}\psi_{L,\gamma_0}\,(-w')
  \;\le\; Z_0\,C_{\rm test}\,\sqrt{E(I)}\cdot L.
\]
Proposition~\ref{prop:Cbox-finite} gives the \textbf{height-independent} bound
$E(I)\le C_{\rm box}\,|I|$ (using the Carleson measure theorem for the inner reciprocal
$\mathcal I$ from Lemma~\ref{lem:inner-reciprocal}).
Hence $\sqrt{E(I)}\le\sqrt{C_{\rm box}|I|}=\sqrt{2C_{\rm box}L}$, and
\begin{equation}\label{eq:upper-CRG}
  Z_0\,C_{\rm test}\,\sqrt{E(I)}\cdot L
  \;\le\; Z_0\,C_{\rm test}\,\sqrt{2C_{\rm box}}\;L^{3/2}
  \;=:\; A\,L^{3/2},
\end{equation}
where $A:=Z_0\,C_{\rm test}\,\sqrt{2C_{\rm box}(\alpha',c)}$ depends only on $(\alpha',c,\psi)$.

\medskip\noindent\emph{Step 3 (contradiction).}
Combining \eqref{eq:lower-zero} and \eqref{eq:upper-CRG}:
\(11\,L\le A\,L^{3/2}\), hence \(11\le A\,L^{1/2}\).
Since \(L=c/\log\angles{\gamma_0}\le c/\log\sqrt{2}\), choosing \(c\) so that
\(A\sqrt{c/\log\sqrt{2}}<11\) (i.e.\ \(c<(11/A)^2\log\sqrt{2}\))
forces a contradiction.
Therefore no such \(\rho_0\) exists, and \(\zeta(s)\neq 0\) for \(\Re s\ge 0.6\).

\medskip\noindent\emph{Small-height case ($|\gamma_0|\le T_0$).}
For any finite height $T_0$, the finite region $\{|\gamma|\le T_0,\;\Re s\ge 0.6\}$
is covered by the shipped ball-arithmetic rectangle certificate (Table~\ref{tab:artifact-data})
for $T_0=20$.
Alternatively, for $|\gamma_0|\le T_0$ one may choose $c$ small enough
that $L\le 1$ and $A\sqrt{L}<11$ hold simultaneously.
\end{proof}
\begin{remark}[Alternative route via the boundary wedge \textup{(P+)}]\label{rem:Pplus-route}
The same conclusion follows (with the same inputs) by the boundary-wedge route
of \S\ref{sec:hybrid}--Appendix~\ref{app:pplus-proof}:
Theorem~\ref{thm:Pplus} $\Rightarrow$ Proposition~\ref{prop:herglotz-schur-transport}
$\Rightarrow$ Corollary~\ref{cor:no-poles}.
The direct-contradiction proof above is logically simpler because it avoids the
global boundary-wedge statement \textup{(P+)}, the Smirnov boundary transport,
and the interior Herglotz/Schur mechanism;
it needs only the \emph{localized} phase-velocity lower bound from a single hypothetical zero.
\end{remark}
\begin{table}[H]
\centering
...
We prove a fixed half-plane zero-exclusion for the Riemann zeta function:
...
The direct-contradiction proof of Theorem~\ref{thm:farfield}
chooses the Whitney parameter $c=c_0/\log\angles{\gamma_0}$
(proportional to $1/\log$ of the height of the hypothetical zero),
causing the $\log^2$ factor in the energy to cancel against $|I|\propto 1/\log^2$,
yielding a height-independent ratio of upper to lower bounds
that gives a contradiction for a universal choice of $c_0<(11/A)^2/2$.

\paragraph{Computer assistance and verifiability.}
The core implications are analytic.
The repository provides supplementary rigorous numerical certificates (ball arithmetic)
in Table~\ref{tab:artifact-data} for low-height cross-checks; these are included for
transparency but are not logically required for the all-heights proof
(except possibly for the small-height case $|\gamma_0|<2$, where the explicit constants
have not been tracked; see the proof of Theorem~\ref{thm:farfield}).
...
We now pass to the load-bearing box-energy control used in the wedge assembly.
...
By Part~(2), \(u^*(\tfrac12+it)=0\) for a.e.\ \(t\).
Moreover, \(|\mathcal I(s)|\le C(1+|t|)^N\) for some \(N\) and all
...
By the Phragmén--Lindelöf principle for the half-plane
(e.g.\ Koosis, \emph{The Logarithmic Integral}, Vol.~I, Ch.~III;
or Garnett~\cite[Ch.~I]{GarnettBAF}):
a subharmonic function on \(\Omega\) with boundary trace \(\le 0\) a.e.\
...
\rsadd{%
\begin{proposition}[Height-dependent box-energy bound on Whitney scales]\label{prop:Cbox-finite}
Let $U=\log|\mathcal J_{\rm out}|$ on $\Omega\setminus Z(\zeta)$,
and let $W=-\log|\mathcal I|\ge 0$ be the nonnegative potential from Lemma~\ref{lem:inner-reciprocal},
so that $U=2\log|B|+W$.
For each Whitney interval $I=[t_0-L,t_0+L]$ with $L=c/\log\angles{t_0}$ and
aperture $\alpha'>1$, define the box energy
\[
  E(I)\ :=\ \iint_{Q(\alpha' I)}|\nabla U|^2\,\sigma\,dt\,d\sigma.
\]
Then
\begin{equation}\label{eq:box-energy-height}
  E(I)\ \le\ C_{\rm box}(\alpha',c)\,|I|,
\end{equation}
where $C_{\rm box}(\alpha',c)<\infty$ is a \textbf{height-independent} constant
depending only on $\alpha'$ and the Whitney parameter $c$.

In particular, the windowed-phase product satisfies
\begin{equation}\label{eq:box-L-product}
  \sqrt{E(I)}\cdot L
  \ \le\ \sqrt{C_{\rm box}}\;L^{3/2}
  \ =\ \sqrt{C_{\rm box}}\;\frac{c^{3/2}}{\log^{3/2}\!\angles{t_0}}\,,
\end{equation}
...
By the Riemann--von Mangoldt short-interval bound,
\(B_{\rm near}\) has at most \(A_0+A_1\alpha''c=O_{\alpha'',c}(1)\) factors.
...
The \(O(1)\) Blaschke factors in \(B_{\rm near}\) each contribute
\(O_{\alpha'}(|I|)\) to the weighted Dirichlet energy on \(Q(\alpha'I)\), giving
\[
  E_{\rm near}\ :=\ \iint_{Q(\alpha'I)}|\nabla\log|B_{\rm near}||^2\,\sigma
  \ =\ O_{\alpha',c}(|I|).
\]

\medskip\noindent\emph{Step 2 (Carleson-measure energy bound for $\widetilde W$).}
Rather than bounding the \emph{pointwise} sup of \(\widetilde W\) on \(\partial D\) and applying a gradient estimate,
we bound the \emph{weighted Dirichlet energy} of \(\widetilde W\) directly using the
Carleson measure property of inner functions.

\emph{Key fact} (Carleson measure theorem for inner functions;
see Garnett~\cite[Ch.~IX]{GarnettBAF} or
Cima--Matheson--Ross, \emph{The Cauchy Transform}, Ch.~6).
Let \(g\) be an inner function on~\(\Omega\) with canonical factorization
\(g=e^{i\theta}B_g S_g\).
Then the measure \(|g'/g|^2\,\sigma\,d\sigma\,dt\) is a \textbf{Carleson measure}
on~\(\Omega\), and for every interval \(J\subset\mathbb R\):
\begin{equation}\label{eq:carleson-inner}
  \iint_{Q(J)}|g'/g|^2\,\sigma\,d\sigma\,dt
  \ \le\ C_{\rm Carl}\,\bigl(\#\{\text{zeros of }g\text{ with ordinate in }3J\}
  \ +\ \nu_{S_g}(3J)\bigr),
\end{equation}
where \(\nu_{S_g}\) is the singular measure of \(S_g\), \(3J\) is a fixed dilate of~\(J\),
and \(C_{\rm Carl}\) is a universal constant.

Now, the neutralized field \(\widetilde W=-\log|B_{\rm far}\,S|\)
satisfies \(|\nabla\widetilde W|^2=|(B_{\rm far}S)'/(B_{\rm far}S)|^2\)
on \(D\) (where \(B_{\rm far}S\) is zero-free and hence \(\widetilde W\) is harmonic).
Apply \eqref{eq:carleson-inner} with \(g=B_{\rm far}\,S\) and \(J\) a constant dilate of \(I\):
\begin{equation}\label{eq:energy-carleson}
  \iint_{Q(\alpha'I)}|\nabla\widetilde W|^2\,\sigma
  \ \le\ C_{\rm Carl}\,\bigl(N_{\rm dilate}\ +\ \nu_S(3\alpha'I)\bigr),
\end{equation}
where \(N_{\rm dilate}:=\#\{\text{zeros of }\mathcal I\text{ with ordinate in }3\alpha'I\text{ but not in }\alpha''I\}\)
is the number of \emph{far zeros near the dilated interval}.
By the Riemann--von Mangoldt bound on Whitney scale:
\(N_{\rm dilate}\le A_0+A_1\cdot 3\alpha'|I|\cdot\log\angles{t_0}
=A_0+6A_1\alpha'c=O_{\alpha',c}(1)\).

For the singular inner mass \(\nu_S(3\alpha'I)\):
since \(S\) is part of the inner function \(\mathcal I\) from Lemma~\ref{lem:inner-reciprocal}
and \(|\mathcal I|\le 1\),
the singular measure satisfies the same Carleson-type bound
as the zero-counting measure (see Garnett~\cite[Ch.~IX, Thm.~1.1]{GarnettBAF}).
On Whitney scale, \(\nu_S(3\alpha'I)=O_{\alpha',c}(1)\).

Therefore \eqref{eq:energy-carleson} gives
\[
  \iint_{Q(\alpha'I)}|\nabla\widetilde W|^2\,\sigma
  \ \le\ C_{\rm Carl}\cdot O_{\alpha',c}(1)
  \ =:\ C_4(\alpha',c).
\]
This is a \textbf{height-independent constant}.

\medskip\noindent\emph{Step 3 (assembly).}
Since \(W=\widetilde W-\log|B_{\rm near}|\):
\(|\nabla W|^2\le 2|\nabla\widetilde W|^2+2|\nabla\log|B_{\rm near}||^2\).
And \(U=2\log|B|+W\):
\(|\nabla U|^2\le 2|\nabla(2\log|B|)|^2+2|\nabla W|^2\).
Combining:
\[
  E(I)\ \le\ 4C_4(\alpha',c)\ +\ O_{\alpha',c}(|I|)
  \ \le\ C_{\rm box}(\alpha',c)\,|I|,
\]
where \(C_{\rm box}(\alpha',c)\) is a \textbf{finite constant independent of the height \(t_0\)}.
This is \eqref{eq:box-energy-height}.
For the product \eqref{eq:box-L-product}:
\(\sqrt{E(I)}\le\sqrt{C_{\rm box}\,|I|}=\sqrt{2C_{\rm box}L}\), hence
\[
  \sqrt{E(I)}\cdot L
  \ \le\ \sqrt{2C_{\rm box}}\;L^{3/2}
  \ =\ \sqrt{2C_{\rm box}}\;\frac{c^{3/2}}{\log^{3/2}\!\angles{t_0}}\;\to\;0
  \quad\text{as }c\to 0,\text{ uniformly in }t_0.
\]
\end{proof}
}%
...
\[
  ...
  \ \le\ Z_0\,C_{\rm test}\,\sqrt{2C_{\rm box}}\;\frac{c^{3/2}}{\log^{3/2}\!2}
...
\(\sqrt{E(I)}\cdot L\le \sqrt{2C_{\rm box}}\,L^{3/2}=\sqrt{2C_{\rm box}}\,c^{3/2}/\log^{3/2}\!\angles{t_0}\),
so
\[
...
$\sqrt{E(I)}\cdot L = O(c^{3/2}/\log^{3/2}\!\angles{T})$
...
Using the Carleson measure theorem for inner functions (Proposition~\ref{prop:Cbox-finite}),
the energy bound is now \emph{height-independent}: \(E(I)\le C_{\rm box}|I|\).
The direct contradiction closes with a fixed choice of~\(c\).
\end{remark}
