...
excluding poles of $\mathcal J$ throughout $\{\Re s>1/2\}$.
...
(The appendix also develops machinery toward a boundary wedge certificate \textup{(P+)},
but the primary proof does not invoke \textup{(P+)}; see Remark~\ref{thm:Pplus}.)
...
The shipped certificates are supplementary cross-checks and are
not logically required (the small-height case is vacuous; see the proof).
...
The Schur/Herglotz pinch mechanism then excludes poles of $\mathcal J$ on $\{\,\Re s>1/2\,\}$ and hence excludes zeros of $\zeta$ there.

\medskip
\noindent\emph{Supplementary computation (not logically required).}
The shipped ball-arithmetic certificates provide independent cross-checks
on representative low-height rectangles.
The proof itself is purely analytic (the small-height case is vacuous).
...
(The hypothetical zero \(\rho_0\) with \(\delta_0\ge 0.1\gg\alpha''L\)
...
The hypothetical zero \(\rho_0\) has \(\delta_0\ge 0.1>\alpha''L\),
...
By that proposition (with the boundary bound \(M\le C_*\log\angles{\gamma_0}\)
and the interior gradient estimate):
\[
...
\begin{table}[H]
\centering
\caption{Supplementary computational artifacts (cross-checks; not logically required for the proof).}\label{tab:artifact-data}
\small
\begin{tabular}{l l l}
\toprule
...
\paragraph{Unconditional status.}
The singular inner factor $S$ of the inner reciprocal $\mathcal I$ is proved trivial
($S\equiv 1$) in Proposition~\ref{prop:Cbox-finite}, Step~2,
using only the polynomial growth of $\zeta$ on $\Omega$ (convexity bound)
and the standard $N^+$ criterion (Garnett~\cite[Ch.~II]{GarnettBAF}).
...
yielding the unconditional contradiction $11\le A\sqrt{c_0}=11/\sqrt{2}<11$.
...
The shipped ball-arithmetic certificates (Table~\ref{tab:artifact-data})
are included as supplementary cross-checks for transparency and reproducibility.
...
which is equivalent to the Riemann Hypothesis for the open half-plane.
...
The boundary wedge \textup{(P+)} discussed in Remark~\ref{thm:Pplus}
is \textbf{not} established here; the primary proof uses a localized contradiction
that does not require the global \textup{(P+)} statement.
...
The machinery below supports the direct-contradiction proof of Theorem~\ref{thm:farfield};
the global boundary wedge \textup{(P+)} of Remark~\ref{thm:Pplus} is \textbf{not} established here.
...
(since \(\delta_0=\beta_0-\tfrac12\ge 0.1>\alpha'L\) for \(t_0\) large),
