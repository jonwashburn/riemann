...
def _env_metadata() -> Dict[str, Any]:
    """
    Return lightweight environment metadata for audit/reproducibility.

    Keep this JSON-friendly (no complex objects).
    """
    return {
        "python": sys.version,
        "platform": platform.platform(),
        "python_flint": getattr(flint, "__version__", "unknown"),
        "flint_ctx_prec": int(getattr(flint.ctx, "prec", -1)),
    }
...
class OuterNormalizerEngineCanonicalStub(OuterNormalizerEngineLike):
    ...
    Status:
      - We provide a *loader + evaluator* skeleton: if a future tool produces a certified
        boundary cache for the canonical trace u(t) on Re(s)=1/2, this class can evaluate
        the corresponding truncated Cauchy/Poisson integral at interior points.
      - We do NOT yet provide a manuscript-grade way to *build* that cache from scratch,
        because doing so requires handling ζ-zeros on the boundary line (a.e. BMO trace,
        smoothing, singular set control, and explicit truncation bounds).

    See:
      - `Riemann-final/FF_CANONICAL_ARTIFACT_PLAN.md` (design/spec for a manuscript-grade
        canonical outer artifact)
      - `paper1_farfield_v6a_all_heights_plan.md` (GAP 2 breakdown)
    """

    # Truncation parameters for the boundary integral on Re(s)=1/2.
    T: float = 0.0
    n_intervals: int = 0

    # Boundary cache (to be supplied by a future canonical-outer builder):
    # arrays over a partition of [-T,T] in t.
    t_intervals: List[arb] | None = None
    u_intervals: List[arb] | None = None  # u(t) ≈ log|F(1/2 - i t)| or a smoothed surrogate
    ...
    def save_cache(self, path: str) -> None:
        if self.t_intervals is None or self.u_intervals is None or self.dt_intervals is None:
            raise ValueError("canonical outer cache save requested before load/build")
        data = {
            "kind": "OuterNormalizerEngineCanonical",
            "version": 1,
    ...
    def load_cache(self, path: str, *, strict: bool = True) -> None:
        ...
        if not isinstance(data, dict):
            raise ValueError("canonical outer cache is not a JSON object")
        if data.get("kind") != "OuterNormalizerEngineCanonical":
            raise ValueError(f"unexpected canonical outer cache kind: {data.get('kind')!r}")
        if int(data.get("version", -1)) != 1:
            raise ValueError(f"unsupported canonical outer cache version: {data.get('version')!r}")
        ...
        if mismatches and strict:
            raise ValueError("canonical outer cache parameter mismatch:\n  " + "\n  ".join(mismatches))
        if mismatches and not strict:
            _progress(
                "[outer_cache] WARNING: parameter mismatch; using canonical cache anyway:\n  "
                + "\n  ".join(mismatches),
                enabled=True,
            )
        ...
        try:
            ...
            raise ValueError(f"failed to parse canonical outer cache arb data: {e}") from e
        if not (len(t_list) == len(u_list) == len(dt_list)):
            raise ValueError("canonical outer cache lists have inconsistent lengths")
    ...
    def build(self) -> None:
        raise NotImplementedError(
            "canonical outer normalizer build is not implemented. "
            "This engine currently supports only load_cache()+O()/O_box() evaluation. "
            "See Riemann-final/FF_CANONICAL_ARTIFACT_PLAN.md."
        )

    def logO(self, s: acb) -> acb:
        """
        logO_can(s) := (1/(π i)) ∫_{-T}^{T} ( 1/(t - w) - t/(1+t^2) ) u(t) dt,
        w := i(s - 1/2).

        This evaluates the same truncated Cauchy/Poisson integral shape used by the other
        outer engines, but anchored at sigma_ref=1/2.

        IMPORTANT: correctness depends entirely on the meaning/certification of u(t) in the cache.
        """
        assert self.t_intervals is not None and self.u_intervals is not None and self.dt_intervals is not None
        sig = arb(1) / arb(2)
        w = acb(0, 1) * (s - acb(sig))
        total = acb(0)
...
def theta_certify_rect(args: Any) -> Dict[str, Any]:
    ...
    if need_outer:
        min_abs_O_disp = None if min_abs_O == float("inf") else min_abs_O
        if min_abs_O_disp is None:
            print(f"[theta] outer: contains0_hits={O_contains0}, min_abs_lower≈None")
        else:
            print(f"[theta] outer: contains0_hits={O_contains0}, min_abs_lower≈{min_abs_O_disp:.6g}")
    if failures:
...
def zeta_certify_rect(args: Any) -> Dict[str, Any]:
    """
    Fully rigorous (ball arithmetic) attempt to certify:
      - ζ(s) does not vanish on the rectangle cover (so 1/ζ is safe there),
    using adaptive bisection with hard caps so runs cannot explode.

    This is intentionally simpler than `theta_certify_rect` and is useful as a
    direct, gauge-free audit on compact regions.
    """
    if str(args.eval_mode) != "rigorous":
        raise ValueError("--zeta-certify requires --eval-mode rigorous (box evaluation)")
    ...
    # Adaptive refinement parameters (reuse --theta-* flags for consistency).
    init_ns = int(args.theta_init_n_sigma)
    init_nt = int(args.theta_init_n_t)
    min_sigma_w = float(args.theta_min_sigma_width)
    min_t_w = float(args.theta_min_t_width)
    ...
    cancel_pole = bool(getattr(args, "zeta_cancel_pole", False))
    obj_name = "(s-1)ζ(s)" if cancel_pole else "ζ(s)"
    print()
    print(f"[zeta] certify {obj_name} != 0 on:", f"[{R.sigma_min},{R.sigma_max}] × [{R.t_min},{R.t_max}]")
    print(
        "[zeta] caps:",
        f"time_limit={time_limit}s, max_boxes={max_boxes}, min_sigma_w={min_sigma_w}, min_t_w={min_t_w}",
    )
    ...
    while stack:
        ...
        zeta_val = s_box.zeta()
        val = (zeta_val * (s_box - acb(1))) if cancel_pole else zeta_val
        if _denom_contains_zero(val):
            zeta_contains0 += 1
            if ws <= min_sigma_w and wt <= min_t_w:
                failures.append({"reason": "zeta_contains0_min_width", "sigma": [sig0, sig1], "t": [t0, t1]})
                continue
        ...
        min_abs_zeta = min(min_abs_zeta, _abs_lower_acb(val))
        passed += 1
        if args.progress and (passed % int(args.theta_progress_every)) == 0:
            dt = prev_elapsed + (time.time() - t_slice_start)
            print(f"[zeta] passed {passed} boxes (processed {processed}, stack {len(stack)}, elapsed {dt:.1f}s)")

    ok = (len(failures) == 0) and (not stack)
    dt = prev_elapsed + (time.time() - t_slice_start)
    print()
    print(f"[zeta] DONE. ok={ok}  passed_boxes={passed}  processed={processed}  refined_splits={refined}  elapsed={dt:.1f}s")
    print(
        f"[zeta] {obj_name}: contains0_hits={zeta_contains0}, "
        f"min_abs_lower≈{(None if min_abs_zeta==float('inf') else min_abs_zeta):.6g}"
    )
    if failures:
        print(f"[zeta] failures: {len(failures)} (showing up to 5)")
        for f in failures[:5]:
            print("  -", f)
    ...
    out = {
        ...
        "zeta": {
            "object": ("sminus1_zeta" if cancel_pole else "zeta"),
            "contains0_hits": zeta_contains0,
            "min_abs_lower": (None if min_abs_zeta == float("inf") else min_abs_zeta),
        },
        "failures": failures,
    }
    out_path = getattr(args, "zeta_out", None)
    if out_path is not None:
        _write_json(str(out_path), out)
        print(f"[zeta] wrote JSON: {out_path}")
    return out
...
def pick_certify(args: Any) -> None:
    ...
    outer_engine: OuterNormalizerEngineLike | None = None
    if gauge in {"outer_xi", "outer_zeta", "outer_zeta_log", "outer_zeta_proj", "canonical_zeta"}:
        ...
        if gauge == "canonical_zeta":
            if args.outer_cache_load is None:
                raise ValueError(
                    "--pick-certify with --arith-gauge canonical_zeta requires --outer-cache-load "
                    "(a precomputed canonical-outer boundary cache; see Riemann-final/FF_CANONICAL_ARTIFACT_PLAN.md)"
                )
            outer_engine = OuterNormalizerEngineCanonicalStub(
                T=float(args.outer_T),
                n_intervals=int(args.outer_n),
            )
            outer_engine.load_cache(
            ...
            # Optional save: re-emit cache after normalization/validation.
            if args.outer_cache_save is not None:
                outer_engine.save_cache(str(args.outer_cache_save))
        elif gauge == "outer_xi":
            outer_engine = OuterNormalizerEngine(
    ...
    # Tail diagnostics: Partial L1 sum of coefficients (n >= N).
    # This is used to check the condition 2 * sum(|a_n|) < delta_cert.
    tail_l1_partial_hi = 0.0
    for n in range(N, Ncoeff):
        an_abs_hi = _safe_abs_upper(coeffs[n])
        tail_l1_partial_hi += float(an_abs_hi)

    # Legacy: Weighted L2 tail (Dirichlet). Kept for compatibility but deprecated for proof.
    ...
    _write_json(
        str(out_path),
        {
            ...
            "pick": {
                ...
                "tail_l1_partial_hi": float(tail_l1_partial_hi),
...
def main() -> None:
    ...
    ap.add_argument(
        "--zeta-certify",
        action="store_true",
        help="Certify ζ(s) != 0 on the given rectangle via adaptive refinement (rigorous).",
    )
    ap.add_argument(
        "--zeta-cancel-pole",
        action="store_true",
        help="For rectangles near s=1, certify (s-1)ζ(s) != 0 instead of ζ(s) != 0 (avoids pole blow-up).",
    )
    ap.add_argument(
        "--theta-certify-cover",
        type=str,
    ...
    ap.add_argument("--zeta-out", type=str, default=None, help="If set, write zeta-certify JSON artifact here.")
    ap.add_argument(
        "--theta-resume-in",
        type=str,
        default=None,
    ...
    if args.theta_certify_cover is not None:
        ...
        return
    if getattr(args, "zeta_certify", False):
        zeta_certify_rect(args)
        return
    ...
    if args.outer_build_only:
        if args.arith_gauge not in {
            ...
            raise ValueError(
                "--outer-build-only requires --arith-gauge outer_xi, outer_zeta, outer_zeta_log, outer_zeta_proj, or canonical_zeta"
            )

        outer_engine: OuterNormalizerEngineLike
        if args.outer_mode == "rigorous":
            if args.arith_gauge == "canonical_zeta":
                if args.outer_cache_load is None:
                    raise ValueError(
                        "--outer-build-only with --arith-gauge canonical_zeta requires --outer-cache-load "
                        "(precomputed canonical outer cache; build not implemented)"
                    )
                outer_engine = OuterNormalizerEngineCanonicalStub(
                    T=float(args.outer_T),
                    n_intervals=int(args.outer_n),
            ...
            if args.arith_gauge == "canonical_zeta":
                raise ValueError("--outer-build-only does not support --arith-gauge canonical_zeta with --outer-mode midpoint")
        ...
        if args.outer_cache_load is not None:
            if isinstance(outer_engine, OuterNormalizerEngineZetaRigorous):
                ...
                print(f"[outer] loaded. leaves={len(outer_engine.t_intervals or [])}")
            elif isinstance(outer_engine, OuterNormalizerEngineCanonicalStub):
                print(f"[outer] loading canonical cache {args.outer_cache_load} ...")
                outer_engine.load_cache(
                    args.outer_cache_load, strict=not bool(args.outer_cache_ignore_mismatch)
                )
                print(f"[outer] loaded. leaves={len(outer_engine.t_intervals or [])}")
                ...
                raise ValueError(
                    "--outer-cache-load is only supported for rigorous outer_zeta/outer_zeta_proj/canonical_zeta or midpoint outer_zeta_log/outer_zeta_proj"
                )
        else:
            print(
        ...
        if args.outer_cache_save is not None:
            if isinstance(outer_engine, OuterNormalizerEngineZetaRigorous):
                ...
                outer_engine.save_cache(args.outer_cache_save)
                print(f"[outer] saved canonical cache to {args.outer_cache_save}")
            elif isinstance(outer_engine, OuterNormalizerEngineMidpointZetaLog):
                outer_engine.save_cache(args.outer_cache_save)
                print(f"[outer] saved cache to {args.outer_cache_save}")
            elif isinstance(outer_engine, OuterNormalizerEngineMidpointZetaProject):
                ...
                raise ValueError(
                    "--outer-cache-save is only supported for rigorous outer_zeta/outer_zeta_proj/canonical_zeta or midpoint outer_zeta_log/outer_zeta_proj"
                )
