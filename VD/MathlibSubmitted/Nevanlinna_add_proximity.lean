...
/-- Monotonicity of circle averages: if `f ≤ g` pointwise on the circle, then
`circleAverage f ≤ circleAverage g`. -/
theorem circleAverage_mono
    {f g : ℂ → ℝ} {c : ℂ} {R : ℝ}
    (hf : CircleIntegrable f c R) (hg : CircleIntegrable g c R)
    (hfg : ∀ x ∈ Metric.sphere c |R|, f x ≤ g x) :
    circleAverage f c R ≤ circleAverage g c R := by
  -- Consider `h = f - g`, which is ≤ 0 on the circle.
  have h_int : CircleIntegrable (fun z ↦ f z - g z) c R := by
    -- `f - g = f + (-g)`
    have hneg : CircleIntegrable (fun z ↦ -g z) c R :=
      (CircleIntegrable.neg (f := g) (c := c) (R := R) hg)
    simpa [sub_eq_add_neg] using
      (CircleIntegrable.add (f := f) (g := fun z => -g z)
        (c := c) (R := R) hf hneg)
  have h_le_zero : ∀ x ∈ Metric.sphere c |R|, (fun z ↦ f z - g z) x ≤ 0 := by
    intro x hx
    have hx' : f x ≤ g x := hfg x hx
    have : f x - g x ≤ 0 := sub_nonpos.mpr hx'
    simpa using this
  -- Apply the scalar monotonicity lemma with constant `0`.
  have h_avg_le_zero :
      circleAverage (fun z ↦ f z - g z) c R ≤ 0 :=
    Real.circleAverage_mono_on_of_le_circle (c := c) (R := R) h_int h_le_zero
  -- Rewrite in terms of `circleAverage f` and `circleAverage g`.
  have h_sub :
      circleAverage (fun z ↦ f z - g z) c R
        = circleAverage f c R - circleAverage g c R :=
    Real.circleAverage_sub (c := c) (R := R) hf hg
  have h_diff_le_zero :
      circleAverage f c R - circleAverage g c R ≤ 0 := by
    simpa [h_sub] using h_avg_le_zero
  exact sub_nonpos.mp h_diff_le_zero
