...
lemma ZetaNoZerosInBox' (T : ℝ) :
  ...
  have hn (n : ℕ) := h (x := 1 - 1 / (n + 1)) (sub_lt_self _ (by positivity))
  ...
  · have zetaIsZero : ζ (1 + Complex.I * t₀) = 0 := by
      have cont := @ZetaCont'
      by_contra h
      use h (isClosed_singleton.isSeqClosed this (.comp (cont.continuousAt.comp (eventually_ne_nhds (by field_simp [ht₀]; simp [*])).mono fun and=>.intro ⟨⟩) (ToOneT0.trans (inf_le_left))))
