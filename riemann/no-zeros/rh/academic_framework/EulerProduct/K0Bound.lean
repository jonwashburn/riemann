...
lemma tsum_subtype_mono_of_nonneg
  ...
  have hind : t.indicator g = s.indicator f := by
    ...
    · by_cases hs : n ∈ s
      ...
      · simp [g, Set.indicator_of_mem htmem, Set.indicator_of_not_mem hs]; exact fun a ↦ False.elim (hs a)
...
lemma P2_le_nat_tail_sq :
  ...
  have hfun :
    ...
    funext p
    rw [rpow_neg, ← Real.rpow_natCast, one_div]
    exact rfl
...
lemma tail_one_div_sq_lt_two_thirds :
  ...
  have tail_eq : (∑' k : {n // 2 ≤ n}, (1 : ℝ) / (k : ℝ) ^ 2) = Real.pi ^ 2 / 6 - 1 := by
    ...
    have h2 : ∑' n : ℕ, (1 : ℝ) / (n : ℝ) ^ 2 =
      ...
      rw [← tsum_add (hsum.indicator (s := {n | 2 ≤ n})) (hsum.indicator (s := {n | n < 2}))]
      congr 1
      ext n
      by_cases h : 2 ≤ n
...
-- Final numerically clean bound: K₀ ≤ 1/8
