...
lemma Gamma_le_Gamma_one_half {a : ℝ} (ha_low : 1/2 ≤ a) (ha_high : a ≤ 1) :
  ...
  calc Real.Gamma a
    ...
    _ ≤ t * Real.Gamma (1/2) + (1-t) * Real.Gamma (1/2) := by
        gcongr
        · exact sub_nonneg_of_le ht_le_one
        · have hΓhalf : 1 ≤ Real.Gamma (1 / 2) := by
            rw [h_half]
...
lemma norm_Complex_Gamma_le_of_re_ge' {w : ℂ} {a : ℝ}
  ...
  have h_int_real :
    ...
    have hcpow : ‖(t : ℂ) ^ (w - 1)‖ = t ^ (w.re - 1) := by
      ...
      exact Complex.abs_cpow_eq_rpow_re_of_pos ht (w - 1)
    have hexp : ‖Complex.exp (-t)‖ = Real.exp (-t) := by
      simp [Complex.norm_eq_abs, Complex.abs_exp, Complex.ofReal_neg]
    simp [norm_mul, hcpow, hexp]
...
lemma norm_Complex_Gamma_le_of_re_ge {w : ℂ} {a : ℝ}
  ...
  have h_big :
    ...
    calc ‖Complex.Gamma w‖
        = ‖∫ t in Ioi (0 : ℝ), f t‖ := h_norm
      _ ≤ ∫ t in Ioi (0 : ℝ), ‖f t‖ := h_le_int
      _ = ∫ t in Ioi (0 : ℝ), g t := h_int_real
      _ = (∫ t in Ioc 0 1, g t) + (∫ t in Ioi 1, g t) := h_split
      _ ≤ (∫ t in Ioc 0 1, t ^ (w.re - 1)) + (∫ t in Ioi 1, g t) := by
