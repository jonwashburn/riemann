...
lemma sum_energy_from_carleson_and_indicator_overlap
  ...
  (hKξ_nonneg : 0 ≤ Kξ) (_hC_nonneg : 0 ≤ C) :
  (∑ i in S, E i) ≤ Kξ * C * Whitney.length I := by
  classical
  -- Sum local Carleson
  have hE_sum : (∑ i in S, E i) ≤ (∑ i in S, Kξ * Whitney.shadowLen (Q i)) :=
    Finset.sum_le_sum (by intro i hi; exact hCar_local i hi)
  -- Factor constants and use overlap
  have hsum_eq :
      (∑ i in S, Kξ * Whitney.shadowLen (Q i)) =
      Kξ * (∑ i in S, Whitney.shadowLen (Q i)) :=
    (Finset.mul_sum (s := S) (f := fun i => Whitney.shadowLen (Q i)) (a := Kξ)).symm
  have hbound' :
      Kξ * (∑ i in S, Whitney.shadowLen (Q i)) ≤ Kξ * (C * Whitney.length I) :=
    mul_le_mul_of_nonneg_left hOverlap hKξ_nonneg
  have hsum_le :
      (∑ i in S, Kξ * Whitney.shadowLen (Q i)) ≤ Kξ * C * Whitney.length I := by
    calc
      (∑ i in S, Kξ * Whitney.shadowLen (Q i))
          = Kξ * (∑ i in S, Whitney.shadowLen (Q i)) := hsum_eq
      _ ≤ Kξ * (C * Whitney.length I) := hbound'
      _ = Kξ * C * Whitney.length I := by
        simp [mul_assoc, mul_comm, mul_left_comm]
  exact le_trans hE_sum hsum_le
...
@[simp] theorem local_pairing_bound_from_Carleson_budget
  {Kξ lenI : ℝ}
  (_hCar : RH.Cert.ConcreteHalfPlaneCarleson Kξ)
...
@[simp] theorem local_pairing_bound_from_IBP_aeZero_and_Carleson
  ...
  have hEqDecomp' : (∫ x in Q, (gradU x) ⋅ (gradChiVpsi x) ∂σ)
    ...
    calc
      (∫ x in Q, (gradU x) ⋅ (gradChiVpsi x) ∂σ)
          = (∫ t in I, ψ t * B t) + Rside + Rtop + Rint := hEqDecomp
      _ = (∫ t in I, ψ t * B t) + 0 + 0 + Rint := by
        simp [hSideZero, hTopZero]
