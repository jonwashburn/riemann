...
theorem I1Bound
  ...
  calc |π|⁻¹ * 2⁻¹ * ‖∫ (t : ℝ) in Iic (-T), SmoothedChebyshevIntegrand SmoothingF eps X (1 + (↑(Real.log X))⁻¹ + ↑t * I)‖
      ≤ |π|⁻¹ * 2⁻¹ * (rexp 1 * K * M * X * Real.log X / (eps * T)) := Z4
    _ = |π|⁻¹ * 2⁻¹ * (rexp 1 * K * M) * X * Real.log X / (eps * T) := by ring
