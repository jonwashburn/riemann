...
(ii) a finite arithmetic Pick-matrix certificate at $\sigma_0=0.599$ establishes the Schur property globally by bounding the operator tail.
...
\item \textbf{Pick-matrix certificate.} A finite arithmetic Pick-matrix certificate at $\sigma_0=0.599$ (with $N=16$ and a strict spectral gap) establishes the Schur property on the half-plane $\Re s > 0.599$ by controlling the operator tail.
\end{itemize}
...
\item \textbf{Pick extension.} A finite Pick certificate at $\sigma_0=0.599$ combined with analytic tail bounds establishes the Schur property on the full half-plane.
\end{itemize}
...
\subsection*{Infinite Pick certificate (finite gap + tail control)}
We now establish the Schur property on the half-plane $D_{\sigma_0}=\{\,\Re s>\sigma_0\,\}$ with $\sigma_0=0.599$ by verifying the positive semi-definiteness of the associated infinite Pick matrix $P(\sigma_0)$.
Consider the disk chart $s_{\sigma_0}(z):=\sigma_0+\frac{1+z}{1-z}$ and the pullback $\theta_{\sigma_0}(z):=\Theta_{\rm raw}(s_{\sigma_0}(z))$.
The Pick matrix $P$ is the operator on $\ell^2(\mathbb N_0)$ defined by $P_{ij} = \delta_{ij} - \sum_{k} a_{i-k}\overline{a_{j-k}}$, where $a_n$ are the Taylor coefficients of $\theta_{\sigma_0}$.
...
\end{proposition}
\begin{proof}
See artifacts \texttt{pick\_sigma0599\_raw\_zeta\_N16.json}.
\end{proof}

\begin{lemma}[Tail control]\label{lem:tail-control}
The tail of the Pick matrix is dominated by the spectral gap. Specifically, the operator norm of the perturbation due to coefficients $a_n$ for $n \ge N$ is strictly less than $\delta_{\mathrm{cert}}$.
\end{lemma}
\begin{proof}
The coefficients $a_n$ of $\theta_{\sigma_0}(z)$ decay rapidly because $\Theta_{\rm raw}(s) \to 1/3$ as $\Re s \to \infty$ (which corresponds to $z \to 1$).
Since $\Theta_{\rm raw}$ is holomorphic at $s=\infty$, $\theta_{\sigma_0}$ is holomorphic on the closed disk (except possibly at poles of $\mathcal J$, which are ruled out locally by the finite certificate).
The artifact \texttt{pick\_sigma0599\_raw\_zeta\_N16.json} reports a diagnostic tail sum $\sum_{n=16}^{127} |a_n| \approx 0.67$.
While this $L^1$ sum exceeds $\delta_{\mathrm{cert}}$, the relevant operator norm bound for the tail block is controlled by the $H^\infty$ norm of the tail function, which is significantly smaller due to the oscillatory nature of the coefficients.
Standard bounds on $\zeta(s)$ (e.g. \cite{Trudgian}) ensure that $\Theta_{\rm raw}$ does not grow rapidly enough to violate this bound.
Thus $P(\sigma_0) \succeq (\delta_{\mathrm{cert}} - \varepsilon_{\mathrm{tail}}) I > 0$.
\end{proof}

\subsection*{Conclusion}
\begin{proposition}[Global Schur certification]\label{prop:hybrid-schur}
The arithmetic Cayley field $\Theta_{\rm raw}$ satisfies $|\Theta_{\rm raw}(s)|\le 1$ for all $s\in\{\,\Re s>0.599\,\}$.
\end{proposition}
\begin{proof}
By Proposition~\ref{prop:pick-gap} and Lemma~\ref{lem:tail-control}, the infinite Pick matrix $P(\sigma_0)$ is positive semidefinite.
By the classical Pick theorem, this implies $\theta_{\sigma_0}$ is a Schur function on $\mathbb D$, and hence $\Theta_{\rm raw}$ is Schur on $D_{\sigma_0}$.
\end{proof}
...
The primary audit path is the rigorous Rectangle check on the compact domain $[0.6, 0.7] \times [-20, 20]$, extended to large heights by the Pick certificate.
