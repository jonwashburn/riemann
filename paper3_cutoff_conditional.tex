...
For each local pair $(L,t_0)$ in the regime of interest, let $\Phi_{L,t_0}:\R\to\C$ denote the test function used on the prime side, with Fourier transform
\[
...
The cutoff hypothesis removes the height-growth mechanism in the Carleson budget and converts the effective barrier into a uniform one.
The following statement records the near-field implication in the form used in the split project.

\begin{theorem}[Nyquist cutoff $\Rightarrow$ uniform near-field exclusion]\label{thm:nyquist-nearfield}
Assume Hypothesis~\ref{hyp:nyquist-cutoff} and assume the cutoff test family satisfies the uniformity/admissibility conditions invoked in the barrier closure step (Section~\ref{sec:closure}); in particular, assume the uniform bound $M_\Phi<\infty$ of Lemma~\ref{lem:nyquist-implies-blocker}.
...
Assume Hypothesis~\ref{hyp:nyquist-cutoff} and assume the uniformity/admissibility conditions used to apply Theorem~\ref{thm:nyquist-nearfield} (in particular, $M_\Phi<\infty$ as in Lemma~\ref{lem:nyquist-implies-blocker}).
...
\subsection*{Roadmap}
Section~\ref{sec:blocker} formalizes the prime-frequency observable and shows that Hypothesis~\ref{hyp:nyquist-cutoff} collapses it to a finite prime sum, yielding a \emph{uniform} arithmetic blocker.
Section~\ref{sec:carleson} converts this into a height-uniform Carleson-box bound.
Finally, Section~\ref{sec:closure} combines the resulting uniform budget with the classical barrier inequality (proved in Paper~II of this series) to obtain Theorem~\ref{thm:nyquist-nearfield} and hence Corollary~\ref{cor:conditional-rh}.
...
and if $M_\Phi<\infty$ (for example if $\sup_{(L,t_0)}\|\Phi_{L,t_0}\|_{L^1}<\infty$, since $|\widehat{\Phi}|\le \|\Phi\|_{L^1}$),
then for all $(L,t_0)$,
\[
...
The additional (conditional) input isolated here is the cutoff-based Carleson estimate of Corollary~\ref{cor:nyquist-carleson}, which is uniform in height for the cutoff prime-frequency field; cf.\ the admissibility caveat in Remark~\ref{rem:blockers}.
...
With the notation above, there exists a finite constant $C(\psi)>0$ (depending only on the fixed window profile and box aperture) such that
...
After local neutralization, one writes $\log F=U+iV$ on the relevant Carleson box and uses the Cauchy--Riemann identity $\partial_t V=\partial_\sigma U$ (in distributions) together with Green's identity to express the boundary pairing against $\psi_{L,\gamma}$ as a Dirichlet pairing.
...
Assume Hypothesis~\ref{hyp:nyquist-cutoff} and the uniform bound $M_\Phi<\infty$ so that Corollary~\ref{cor:nyquist-carleson} applies.
...
Assuming admissibility as in Remark~\ref{rem:blockers}, this removes the height-growth escape in the barrier closure and excludes such a zero for all $\gamma$.
...
Assume Hypothesis~\ref{hyp:nyquist-cutoff} and the uniform bound $M_\Phi<\infty$ so that Theorem~\ref{thm:nyquist-nearfield} applies.
...
Therefore the completed zeta function $\xi(s)$ has no zeros with $\Re s>\tfrac12$.
