...
\[
  C_{\rm box}^{(\zeta)}\ :=\ \sup_{I\ \mathrm{Whitney}}\ \frac{1}{|I|}\iint_{Q(\alpha' I)}|\nabla U|^2\,\sigma\,dt\,d\sigma,
\]
...
\subsection*{Closing the box-energy loop: \(U=\Re\log J_\zeta\) as a Green potential}

\begin{lemma}[Green potential representation for \(U=\Re\log J_\zeta\)]\label{lem:green-potential-Jzeta}
Let \(J_\zeta\) be the \(\zeta\)–normalized unimodular ratio from Lemma~\ref{lem:zeta-normalization}, and set
\[
  U(s)\ :=\ \Re\log J_\zeta(s)\ =\ \log|J_\zeta(s)|
...
\]
Let \(\mu_\xi\) denote the off-critical \(\xi\)-zero counting measure in \(\Omega\),
\[
...
\[
  ...
  \qquad B(s)=\frac{s-1}{s}.
\]
In particular, up to the explicit compensator \(2\log|B|\), the field \(U=\Re\log J_\zeta\) is the Green potential generated by the \(\xi\)-zeros in \(\Omega\).
\end{lemma}
\begin{proof}
Since \(\dettwo(I-A)\) and \(\mathcal O_\zeta\) are holomorphic and nonvanishing on \(\Omega\), the poles of \(J_\zeta\) in \(\Omega\) coincide with the zeros of \(\zeta\) in \(\Omega\), hence with the zeros of \(\xi\) in \(\Omega\).
Moreover, \(J_\zeta\) has a zero at \(s=1\) of order \(2\): one order comes from the simple pole of \(\zeta\) at \(s=1\) (so \(1/\zeta\) has a simple zero) and one order comes from the factor \(B(s)=(s-1)/s\).

We use the standard divisor identity for a meromorphic function \(F\) on a planar domain:
\(\Delta\log|F| = 2\pi\sum_{\text{zeros}} m\,\delta - 2\pi\sum_{\text{poles}} m\,\delta\)
in distributions.
Applying this to \(J_\zeta\) gives
\[
  -\Delta U \ =\ 2\pi\,\mu_\xi\ -\ 4\pi\,\delta_{1}.
\]
Since \(B(s)\) is holomorphic on \(\Omega\) with a simple zero at \(s=1\) and no poles in \(\Omega\), we likewise have \(\Delta\log|B| = 2\pi\,\delta_{1}\), hence
\(-\Delta(2\log|B|)=-4\pi\,\delta_{1}\).
Subtracting yields the displayed identity.
\end{proof}

\begin{proposition}[Whitney-uniform box energy for \(J_\zeta\)]\label{prop:box-energy-Jzeta}
Let \(J_\zeta\) be as in Lemma~\ref{lem:zeta-normalization}, and write \(\log J_\zeta = U+iW\) on \(\Omega\setminus Z(\zeta)\).
Then on the Whitney schedule \(I=[T-L,T+L]\) with \(L=c/\log\angles{T}\) (and with any fixed aperture \(\alpha'>1\)) one has a uniform box-energy bound
\[
  \iint_{Q(\alpha' I)}|\nabla U(\sigma,t)|^2\,\sigma\,dt\,d\sigma\ \le\ C_{\rm box}\,|I|
\]
for some finite constant \(C_{\rm box}=C_{\rm box}(\alpha',c)\).
In particular, the box-energy constant \(C_{\rm box}^{(\zeta)}\) appearing in Proposition~\ref{prop:length-free} is finite when \(J=J_\zeta\) (hence also when \(J=\mathcal J_{\rm out}\) by \eqref{eq:J-out}).
\end{proposition}
\begin{proof}
By Lemma~\ref{lem:green-potential-Jzeta}, up to the explicit compensator \(2\log|B|\), the field \(U=\Re\log J_\zeta\) is generated by the \(\xi\)-zero divisor in \(\Omega\) via the half-plane Green kernel.
On Whitney boxes \(Q(\alpha' I)\), the compensator term is smooth (for \(c\) chosen so that the Whitney clip stays away from \(s=1\)) and contributes \(O(|I|)\) to the weighted Dirichlet energy.
Thus it suffices to bound the Green-potential contribution from the \(\xi\)-zeros.

As in the proof of Lemma~\ref{lem:carleson-xi}, neutralize the finitely many zeros in a fixed dilate of \(Q(\alpha' I)\) by a local half-plane Blaschke product.
This reduces the estimate on \(Q(\alpha' I)\) to bounding an \(L^2(\sigma\,dt\,d\sigma)\) square function built from Poisson kernels centered at the zero ordinates.
Decomposing zeros into Whitney annuli and applying the annular balayage bound (Lemma~\ref{lem:annular-balayage}), together with the short-interval zero count \(N(T;H)\ll 1+H\log\angles{T}\) on Whitney scale \(H\asymp L=c/\log\angles{T}\), yields
\(\iint_{Q(\alpha' I)}|\nabla U|^2\,\sigma\ll_{\alpha',c}|I|\).
\end{proof}
...
By Proposition~\ref{prop:box-energy-Jzeta}, the Whitney box-energy constant \(C_{\rm box}^{(\zeta)}\) for \(J_\zeta\) is finite.
