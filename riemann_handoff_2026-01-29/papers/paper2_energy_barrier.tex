...
\begin{abstract}
This paper develops an effective near-field barrier against off-critical zeros of the Riemann zeta function in the strip $\tfrac12<\Re s<0.6$.
Writing a hypothetical zero as $\rho=\tfrac12+\eta+i\gamma$ with $\eta\in(0,0.1)$, we compare a universal windowed phase-cost lower bound (a half-plane Blaschke trigger) to a scale-tracked Carleson-box energy budget for a harmonic log-modulus potential attached to a locally neutralized zeta-ratio.
This yields an explicit \emph{protection height} $T_{\rm safe}(\eta)$ such that any such zero must satisfy $|\gamma|>T_{\rm safe}(\eta)$.
The bound is enormous but fully explicit; it also isolates the single height-growth mechanism in the budget (a term proportional to $L\log\angles{|\gamma|}$ with $L=2\eta$), explaining why the conclusion is height-limited rather than uniform in height.
\end{abstract}
...
This paper isolates a complementary statement of \emph{effective} (height-limited) type: we derive an explicit height barrier against zeros that lie strictly to the right of the critical line but remain in the near strip
\[
  \tfrac12 < \Re s < 0.6.
\]

\subsection*{Main statement (effective; no all-heights claim)}
Write a hypothetical off-critical zero as $\rho=\tfrac12+\eta+i\gamma$ with $\eta\in(0,0.1)$ and set the natural near-field scale and height parameters
\[
  L\ :=\ 2\eta\in(0,0.2),
  \qquad
  T\ :=\ |\gamma|,
  ...
  \angles{T}\ :=\ \sqrt{1+T^2}.
\]
The main output is a computable \emph{protection height} $T_{\rm safe}(\eta)$ such that an off-critical zero at depth $\eta$ cannot occur below height $T_{\rm safe}(\eta)$.
...
if $\rho=\tfrac12+\eta+i\gamma$ is a zero of $\zeta(s)$ with $\eta\in(0,0.1)$, then necessarily
\[
  |\gamma|\ >\ T_{\rm safe}(\eta).
\]
Equivalently, $\zeta(s)\neq 0$ for every point of the form $s=\tfrac12+\eta+it$ with $\eta\in(0,0.1)$ and $|t|\le T_{\rm safe}(\eta)$.
\end{theorem}
...
\subsection*{Why the result is effective: energy cost versus a scale-tracked Carleson budget}
...
After localizing at the natural scale $L=2\eta$ by a compactly supported window centered at height $\gamma$, this produces a universal \emph{minimum} phase-mass cost
\[
  L_{\rm rec}\ =\ 4\arctan(2)\ \approx\ 4.428.
\]
\item \textbf{Upper bound (Carleson energy budget).}
The same localized phase mass is bounded above by a Carleson-box energy constant for a harmonic log-modulus potential on a box of height comparable to $L$ above the support interval $[\gamma-2L,\gamma+2L]$.
