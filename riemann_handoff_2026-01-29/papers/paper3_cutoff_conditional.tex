...
\begin{abstract}
This paper isolates a single explicit additional assumption---a fixed Nyquist cutoff (bandlimit) for a near-field test family---and records its consequences within the near-field barrier framework.
Under the cutoff and a uniform normalization bound $M_\Phi<\infty$, the prime exponential sum driving the prime-side input becomes a finite sum over primes $p\le e^{\Omega_{\max}}$, yielding a uniform arithmetic blocker and a Carleson-box energy bound for the associated cutoff prime field that is uniform in both scale and height.
Assuming (as an explicit admissibility hypothesis) that the near-field barrier closure of Paper~II may be run within such a cutoff-admissible test family, we obtain a conditional zero-free strip $\tfrac12<\Re s<0.6$.
Combined with the unconditional far-field zero-freeness $\Re s\ge 0.6$ proved in Paper~I, this yields a conditional closure of the Riemann Hypothesis.
No claim is made that the cutoff follows from classical analysis or number theory; the purpose is to separate this hypothesis cleanly from unconditional arguments.
\end{abstract}
...
(ii) an upper bound coming from a Carleson-box energy budget at the corresponding scale $L=2\eta$.
Throughout this paper we restrict to the near-field regime
\[
  \eta\in(0,0.1),
  \qquad
  L:=2\eta\in(0,0.2],
...
For each pair $(L,t_0)$ with $0<L\le 0.2$ and $t_0\in\R$, let $\Phi_{L,t_0}\in L^1(\R)$ denote the test function used on the prime side, with Fourier transform
\[
...
\begin{hypothesis}[Cutoff-admissibility of the barrier test family]\label{hyp:cutoff-admissibility}
In applying the near-field barrier closure argument of Paper~II on the near-field range $0<L\le 0.2$, assume that the argument may be run using a family of test functions $\Phi_{L,t_0}$ which satisfies Hypothesis~\ref{hyp:nyquist-cutoff} for all relevant pairs $(L,t_0)$, together with the uniform bound $M_\Phi<\infty$ as in Lemma~\ref{lem:nyquist-implies-blocker}.
\end{hypothesis}

\begin{theorem}[Nyquist cutoff (admissible) $\Rightarrow$ uniform near-field exclusion]\label{thm:nyquist-nearfield}
Assume Hypothesis~\ref{hyp:nyquist-cutoff} and Hypothesis~\ref{hyp:cutoff-admissibility}, and assume the uniform bound $M_\Phi<\infty$ as in Lemma~\ref{lem:nyquist-implies-blocker}.
...
Assume Hypothesis~\ref{hyp:nyquist-cutoff} and Hypothesis~\ref{hyp:cutoff-admissibility}, and assume $M_\Phi<\infty$ as in Lemma~\ref{lem:nyquist-implies-blocker}.
...
\begin{itemize}
\item \textbf{Admissibility.} Hypothesis~\ref{hyp:cutoff-admissibility} asserts that the barrier argument may be run using only test functions satisfying Hypothesis~\ref{hyp:nyquist-cutoff} for all relevant $(L,t_0)$.
...
Assume Hypothesis~\ref{hyp:nyquist-cutoff}, Hypothesis~\ref{hyp:cutoff-admissibility}, and the uniform bound $M_\Phi<\infty$ so that Corollary~\ref{cor:nyquist-carleson} applies.
...
In the effective analysis of Paper~II, height dependence enters through the available budget at the same scale: in general one obtains an upper bound whose dominant growth in $|\gamma|$ is proportional to $L\log\angles{|\gamma|}$.
...
By Hypothesis~\ref{hyp:cutoff-admissibility}, the barrier closure argument of Paper~II may be carried out within a cutoff-admissible test family using this height-uniform estimate as the needed prime-frequency input.
In particular, the closure step has no large-height escape at fixed $\eta$, and the cost-versus-budget inequality yields a contradiction for every $\gamma$.
...
Assume Hypothesis~\ref{hyp:nyquist-cutoff}, Hypothesis~\ref{hyp:cutoff-admissibility}, and the uniform bound $M_\Phi<\infty$ so that Theorem~\ref{thm:nyquist-nearfield} applies.
