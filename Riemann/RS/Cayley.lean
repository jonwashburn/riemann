...
lemma boundary_abs_J_pinch_eq_one
  ...
  have hden_ne : (norm (O z) * norm (riemannXi_ext z)) ≠ 0 := ne_of_gt hden_pos
...
lemma boundary_Re_F_pinch_le_two
  ...
  have hJ : norm (J_pinch det2 O ((2⁻¹ : ℂ) + Complex.I * (t : ℂ))) = 1 := by
    -- boundary t = 1/2 + i t (definitional), but avoid importing HalfPlaneOuter here
    simpa using hJb
  have hFabs : norm ((F_pinch det2 O) (boundary t)) = (2 : ℝ) := by
    calc
      norm ((F_pinch det2 O) (boundary t))
          = norm ((2 : ℂ) * J_pinch det2 O (boundary t)) := by
              simp [F_pinch]
      _ = norm (2 : ℂ) * norm (J_pinch det2 O (boundary t)) := by
              exact Complex.norm_mul (2 : ℂ) (J_pinch det2 O (boundary t))
      _ = (2 : ℝ) * 1 := by
        have h2 : norm (2 : ℂ) = (2 : ℝ) := by norm_num
        -- hJ says norm (J_pinch det2 O (2⁻¹ + I * ↑t)) = 1
      ...
      _ = (2 : ℝ) := by norm_num
  calc
    |((F_pinch det2 O) (boundary t)).re| ≤ norm ((F_pinch det2 O) (boundary t)) :=
