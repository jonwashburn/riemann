...
lemma psi_nonneg : ∀ t, 0 ≤ psi t := by
  ...
  · simp [psi, Set.indicator_of_notMem ht]
...
theorem poisson_plateau_lower_bound
  ...
  have constJ : (∫ t in Icc (x - b) (x + b), poissonKernelPlateau b (x - t))
    ...
    have hint_c : Integrable ((Icc (x - b) (x + b)).indicator (fun _ => (b⁻¹ * (Real.pi⁻¹ * 2⁻¹)))) := by
      have : IntegrableOn (fun _ => (b⁻¹ * (Real.pi⁻¹ * 2⁻¹))) (Icc (x - b) (x + b)) volume := by
        apply integrableOn_const
        · exact LT.lt.ne_top hμJ
        . exact enorm_ne_top
...
lemma poisson_plateau_c0 :
  ...
  · intro b x hb hb1 hx
    ...
    have hpt : (fun t => poissonKernelPlateau b (x - t) * psi t)
      ...
      funext t
      by_cases ht : t ∈ Icc (-2 : ℝ) 2
      · simp [psi, Set.indicator_of_mem ht, mul_comm]
      · simp [psi, Set.indicator_of_notMem ht]
    ...
    have hindEq_expected :
      ...
      have hptfun :
        ...
        funext t
        by_cases ht : t ∈ Icc (-2 : ℝ) 2
        · simp [Set.indicator_of_mem ht, mul_comm]
        · simp [Set.indicator_of_notMem ht]
    ...
    have constJ : (∫ t in Icc (x - b) (x + b), poissonKernelPlateau b (x - t))
      ...
      have hint : Integrable ((Icc (x - b) (x + b)).indicator (fun t => poissonKernelPlateau b (x - t))) := by
        simpa [integrable_indicator_iff, hmeasJ] using hint_on
      have hint_c : Integrable ((Icc (x - b) (x + b)).indicator (fun _ => (b⁻¹ * (Real.pi⁻¹ * 2⁻¹)))) := by
        have : IntegrableOn (fun _ : ℝ => (b⁻¹ * (Real.pi⁻¹ * 2⁻¹)))
          (Icc (x - b) (x + b)) volume := by
          refine (integrableOn_const_iff ?_).mpr ?_
          · exact enorm_ne_top
          · exact Or.inr (by simp [Real.volume_Icc])
        simpa [integrable_indicator_iff, hmeasJ] using this
      have hpt : (Icc (x - b) (x + b)).indicator (fun _ => (b⁻¹ * (Real.pi⁻¹ * 2⁻¹)))
                  ≤ᵐ[volume]
                  (Icc (x - b) (x + b)).indicator (fun t => poissonKernelPlateau b (x - t)) := by
    ...
    have hscale : c0_plateau ≤ (1/4 : ℝ) * ∫ t in Icc (-2 : ℝ) 2, poissonKernelPlateau b (x - t) := by
      have h := mul_le_mul_of_nonneg_left base2 (by norm_num : 0 ≤ (1 / (4 : ℝ)))
      -- c0_plateau is definitionally 4⁻¹ * π⁻¹, which equals (1/4) * π⁻¹
