...
theorem shadow_overlap_bound_pass
  {ι : Type*} (S : Finset ι)
  (Q : ι → Set (ℝ × ℝ)) (I : Set ℝ)
  (h : (∑ i ∈ S, shadowLen (Q i)) ≤ shadowOverlapConst * length I) :
  (∑ i ∈ S, shadowLen (Q i)) ≤ shadowOverlapConst * length I := h
...
@[simp] lemma WhitneyInterval_interval_length
  ...
  have hIcc_len :
    ...
    have : (volume (Set.Icc (W.t0 - W.len) (W.t0 + W.len))).toReal
      ...
      simp [Real.volume_Icc]
    simp [RH.RS.length, Whitney.length, ENNReal.toReal_ofReal, hΔ, hlen_nonneg]
