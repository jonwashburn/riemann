...
import Mathlib.MeasureTheory.Measure.Lebesgue.Basic
import Riemann.RS.WhitneyGeometryDefs
open RH.Cert RH.Cert.KxiWhitneyRvM RH.RS
...
lemma annularEnergy_le_S_times_diag
  ...
  refine set_integral_mono_on_nonneg (s := Set.Ioc (0 : ℝ) (α * I.len)) (μ := volume)
    (f := fun σ => (∫ t in I.interval, (∑ γ ∈ Zk, KxiWhitneyRvM.Ksigma σ (t - γ)) ^ 2) * σ)
    (g := fun σ => (h.S * (∫ t in I.interval,
                            σ * (∑ γ ∈ Zk, (KxiWhitneyRvM.Ksigma σ (t - γ)) ^ 2))))
    ?hfin ?hfin' ?hAE
  ...
  · -- Almost-everywhere pointwise inequality for σ ∈ (0, αL]
    ...
    have h_inner :
      ...
      have :
        (∑ γ ∈ Zk, KxiWhitneyRvM.Ksigma σ (t - γ)) ^ 2
          = (∑ γ ∈ Zk, KxiWhitneyRvM.Ksigma σ (t - γ))
            * (∑ γ' ∈ Zk, KxiWhitneyRvM.Ksigma σ (t - γ')) := by
        ring
      ...
      have := (integral_sum (s := Zk) (μ := volume)
        (f := fun γ t => (∑ γ' ∈ Zk, KxiWhitneyRvM.Ksigma σ (t - γ'))
            * KxiWhitneyRvM.Ksigma σ (t - γ)))
...
lemma Zk_separated_from_base
  ...
  classical
  intro γ hγ t ht
  -- Membership ∈ Zk implies the annulus predicate
...
/-- Logarithmic potential built from the analytic reciprocal `G`. -/
noncomputable def U_field (p : ℝ × ℝ) : ℝ :=
  let s : ℂ := ((1 / 2 : ℝ) + p.2 : ℂ) + Complex.I * (p.1 : ℂ)
  have hs : s ∈ ΩoffXi := by
    -- supply: (p.2 > 0 ⇒ s ∈ Ω) and `riemannXi_ext s ≠ 0` in the tents being used
    sorry
  (logG ⟨s, hs⟩).re
