...
lemma cs_sum_sq_finset {ι : Type*} [DecidableEq ι] (s : Finset ι) (f : ι → ℝ) :
  ...
  · -- Direct calculation using sum expansion
    ...
    calc (∑ i ∈ s, f i) ^ 2
        = (∑ i ∈ s, f i) * (∑ j ∈ s, f j) := by ring
      _ = ∑ i ∈ s, ∑ j ∈ s, f i * f j := by rw [Finset.sum_mul_sum]
      _ ≤ ∑ i ∈ s, ∑ j ∈ s, (f i ^ 2 + f j ^ 2) / 2 := by
          gcongr with i _ j _
      ...
      _ = s.card * ∑ i ∈ s, f i ^ 2 := by
          ...
          have h1 : ∑ i ∈ s, ∑ j ∈ s, (f i ^ 2 + f j ^ 2) / 2
                  = ∑ i ∈ s, ∑ j ∈ s, f i ^ 2 / 2 + ∑ i ∈ s, ∑ j ∈ s, f j ^ 2 / 2 := by
            rw [← Finset.sum_add_distrib]
            congr 1; ext i; rw [← Finset.sum_add_distrib]; congr 1; ext j
            simp_rw [add_div]
          rw [h1]
          have h2 : ∑ i ∈ s, ∑ j ∈ s, f i ^ 2 / 2 = (s.card : ℝ) * ∑ i ∈ s, f i ^ 2 / 2 := by
            rw [Finset.sum_comm, Finset.sum_const, nsmul_eq_mul]
          have h3 : ∑ i ∈ s, ∑ j ∈ s, f j ^ 2 / 2 = (s.card : ℝ) * ∑ i ∈ s, f i ^ 2 / 2 := by
...
@[simp] noncomputable def annularEnergyDiag (α : ℝ) (I : WhitneyInterval) (Zk : Finset ℝ) : ℝ :=
  ∫ σ in Set.Ioc (0 : ℝ) (α * I.len),
    (∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂(volume)) * σ ∂(volume)
...
lemma annularEnergy_nonneg (α : ℝ) (I : WhitneyInterval) (Zk : Finset ℝ) :
  ...
  exact inner_energy_nonneg α I Zk
...
open scoped BigOperators
lemma pow_le_pow_of_le_left {α : Type*} [Field α] [LinearOrder α] [IsStrictOrderedRing α]
  {a b : α} (h₁ : a ≤ b) (h₂ : 0 ≤ a) :
  ∀ n : ℕ, a ^ n ≤ b ^ n := by
  intro n
...
lemma integrable_ksigma_sq (σ x : ℝ) (hσ : 0 < σ) :
  ...
  have h_fin : (∫⁻ a, ENNReal.ofReal ((Ksigma σ (a - x))^2)) ≠ ⊤ := by
    intro h_top
    rw [h_top, ← EReal.toENNReal_top] at h_eq
...
lemma poisson_integral_measurable_in_param (σ_bound : ℝ) (hσ_bound : 0 < σ_bound)
  ...
  have : (fun σ => ∫ t in I, (Vk Zk σ t)^2 ∂volume) =
         (fun σ => ∫ t in I, (∑ γ ∈ Zk, σ / ((t - γ)^2 + σ^2))^2 ∂volume) := by
    ext σ
    congr 1
  rw [this]
  ...
  have h_rewrite :
    ...
    have hK2 :
      ...
      calc
        ...
        _   = (1 / σ) * (1 / (u ^ 2 + 1)) := by field_simp [hσne]
...
lemma poisson_integral_diagonal_measurable_in_param (σ_bound : ℝ) (hσ_bound : 0 < σ_bound)
    (I : Set ℝ) (hI : MeasurableSet I) (hI_bounded : Bornology.IsBounded I) (Zk : Finset ℝ) :
    AEStronglyMeasurable (fun σ => ∫ t in I, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume)
      (Measure.restrict volume (Set.Ioc (0 : ℝ) σ_bound)) := by
  -- Finite sums preserve measurability, so reduce to the singleton case
  have h_expand : (fun σ => ∫ t in I, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume) =
      (fun σ => ∑ γ ∈ Zk, ∫ t in I, (Ksigma σ (t - γ))^2 ∂volume) := by
    ext σ
    ...
    · -- Continuous function on finite-measure set is integrable
      ...
      have hfin :
          HasFiniteIntegral (fun t => (Ksigma σ (t - γ))^2)
            (Measure.restrict volume I) :=
              HasFiniteIntegral.restrict_of_bounded (1 / σ ^ 2) hI_finite hAE
  ...
  refine Finset.aestronglyMeasurable_fun_sum
    (s := Zk)
    (f := fun γ (σ : ℝ) => ∫ t in I, (Ksigma σ (t - γ))^2 ∂volume)
    (fun γ _ => ?_)
  -- For singleton {γ}, use the existing machinery
  have hEq :
      (fun σ : ℝ => ∫ t in I, (Ksigma σ (t - γ))^2 ∂volume) =
        (fun σ => ∫ t in I, (Vk {γ} σ t)^2 ∂volume) := by
    ext σ
    simp [Vk, Ksigma]
  have hVk :
      AEStronglyMeasurable (fun σ => ∫ t in I, (Vk {γ} σ t)^2 ∂volume)
        (Measure.restrict volume (Set.Ioc (0 : ℝ) σ_bound)) :=
    poisson_integral_measurable_in_param σ_bound hσ_bound I hI hI_bounded {γ}
...
theorem annularEnergy_le_card_mul_diag
  ...
  have hpt (σ t : ℝ) :
    (Vk Zk σ t)^2 ≤ (Zk.card : ℝ) * (∑ x ∈ Zk, (Ksigma σ (t - x))^2) := by
  ...
  have hσ (σ : ℝ) (hσmem : σ ∈ Set.Ioc (0 : ℝ) (α * I.len)) :
    (∫ t in I.interval, (Vk Zk σ t)^2 ∂(volume)) * σ
      ≤ (∫ t in I.interval, ((Zk.card : ℝ) * (∑ x ∈ Zk, (Ksigma σ (t - x))^2)) ∂(volume)) * σ := by
    have hAE :
      (fun t => (Vk Zk σ t)^2)
        ≤ᵐ[Measure.restrict volume I.interval]
      (fun t => (Zk.card : ℝ) * (∑ x ∈ Zk, (Ksigma σ (t - x))^2)) :=
      Filter.Eventually.of_forall (fun t => hpt σ t)
    have hInt :
      ∫ t in I.interval, (Vk Zk σ t)^2 ∂(volume)
        ≤ ∫ t in I.interval, ((Zk.card : ℝ) * (∑ x ∈ Zk, (Ksigma σ (t - x))^2)) ∂(volume) := by
      ...
      have h_int2 : Integrable (fun t => (Zk.card : ℝ) * (∑ x ∈ Zk, (Ksigma σ (t - x))^2))
        (Measure.restrict volume I.interval) := by
        have hsum_cont : Continuous (fun t =>
            ∑ x ∈ Zk, (Ksigma σ (t - x))^2) := by
          apply continuous_finset_sum
          intro x _hx
          have hσpos : 0 < σ := hσmem.1
  ...
  have hAEσ :
    ...
    simpa using
      (ae_restrict_iff' (μ := volume)
        ...
        (p := fun σ =>
          (∫ t in I.interval, (Vk Zk σ t)^2 ∂(volume)) * σ
            ≤ (∫ t in I.interval, ((Zk.card : ℝ) * (∑ x ∈ Zk, (Ksigma σ (t - x))^2)) ∂(volume)) * σ)
        hmeasσ).mpr hAEσ'
  ...
  have h_int1 :
    ...
    have h_bound :
      ...
      have hCS :
        ∫ t in I.interval, (Vk Zk σ t)^2 ∂(volume)
          ≤ (Zk.card : ℝ) *
              ∑ x ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - x))^2 ∂(volume) := by
        have h_int_lhs : Integrable (fun t => (Vk Zk σ t)^2) (Measure.restrict volume I.interval) := by
          ...
          exact hcont.continuousOn.integrableOn_compact hIcompact
        have h_int_rhs : Integrable (fun t => (Zk.card : ℝ) * (∑ x ∈ Zk, (Ksigma σ (t - x))^2))
            (Measure.restrict volume I.interval) := by
          have hcont : Continuous (fun t => (Zk.card : ℝ) * (∑ x ∈ Zk, (Ksigma σ (t - x))^2)) := by
        ...
        calc ∫ t in I.interval, (Vk Zk σ t)^2 ∂volume
          ...
          _ = (Zk.card : ℝ) * ∫ t in I.interval, (∑ x ∈ Zk, (Ksigma σ (t - x))^2) ∂volume := by
              rw [MeasureTheory.integral_const_mul]
          _ = (Zk.card : ℝ) * ∑ x ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - x))^2 ∂volume := by
              congr 1
      ...
      have : (∫ t in I.interval, (Vk Zk σ t)^2 ∂volume) * σ
          ≤ (Zk.card : ℝ) * (∑ x ∈ Zk, (Real.pi / 2)) := by
        calc (∫ t in I.interval, (Vk Zk σ t)^2 ∂volume) * σ
            ≤ ((Zk.card : ℝ) * ∑ x ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - x))^2 ∂volume) * σ :=
              mul_le_mul_of_nonneg_right hCS (le_of_lt hσpos)
          _ = (Zk.card : ℝ) * (∑ x ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - x))^2 ∂volume) * σ := by ring
          _ ≤ (Zk.card : ℝ) * (∑ x ∈ Zk, (Real.pi / 2)) := by
              have : (∑ x ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - x))^2 ∂volume) * σ
                  ≤ ∑ x ∈ Zk, (Real.pi / 2) := by
                have : ∀ x ∈ Zk, (∫ t in I.interval, (Ksigma σ (t - x))^2 ∂volume) * σ ≤ Real.pi / 2 :=
                  fun x hx => h_piece x hx
                calc (∑ x ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - x))^2 ∂volume) * σ
                    = ∑ x ∈ Zk, (∫ t in I.interval, (Ksigma σ (t - x))^2 ∂volume) * σ := by
                      rw [Finset.sum_mul]
                  _ ≤ ∑ x ∈ Zk, (Real.pi / 2) :=
      ...
      calc ‖∫ t in I.interval, (Vk Zk σ t)^2 ∂volume‖ * σ
        ...
        _ ≤ (Zk.card : ℝ) * (∑ x ∈ Zk, (Real.pi / 2)) := this
    ...
    · -- Bounded integral on finite measure
      apply HasFiniteIntegral.of_bounded (C := (Zk.card : ℝ)^2 * (Real.pi / 2))
  ...
  have h_int2 :
    ...
    have h_bound :
      ...
      have h_piece :
        ...
        calc (∫ t in I.interval, (Ksigma σ (t - x))^2 ∂volume) * σ
          ...
          _ = ((Real.pi / 2) / σ) * σ := by rw [h_all]
          _ = (Real.pi / 2) := by field_simp
      calc ‖∫ t in I.interval, ((Zk.card : ℝ) * (∑ x ∈ Zk, (Ksigma σ (t - x))^2)) ∂volume‖ * σ
          ≤ ((Zk.card : ℝ) * ∑ x ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - x))^2 ∂volume) * σ := by
            gcongr
            rw [Real.norm_eq_abs, abs_of_nonneg]
            · rw [MeasureTheory.integral_const_mul, integral_finset_sum]
              intro x _
              have hcont : Continuous (fun t => (Ksigma σ (t - x))^2) := by
                unfold Ksigma
                have hden_cont : Continuous (fun t => (t - x) ^ 2 + σ ^ 2) := by
    ...
    · -- Measurability
      ...
      · -- The integral part is measurable
        have heq : (fun σ => ∫ t in I.interval, ((Zk.card : ℝ) * (∑ x ∈ Zk, (Ksigma σ (t - x))^2)) ∂volume) =
                   (fun σ => (Zk.card : ℝ) * ∫ t in I.interval, (∑ x ∈ Zk, (Ksigma σ (t - x))^2) ∂volume) := by
          funext σ; rw [MeasureTheory.integral_const_mul]
        rw [heq]
        ...
        · have hI_bounded : Bornology.IsBounded I.interval := by
          ...
          have : AEStronglyMeasurable
            (fun σ => ∫ t in I.interval, (∑ x ∈ Zk, (Ksigma σ (t - x))^2) ∂volume)
            (Measure.restrict volume (Set.Ioc (0 : ℝ) (α * I.len))) := by
    ...
    · -- Bounded integral on finite measure
      apply MeasureTheory.HasFiniteIntegral.of_bounded (C := (Zk.card : ℝ)^2 * (Real.pi / 2))
      refine (ae_restrict_iff' measurableSet_Ioc).mpr ?_
      exact Filter.Eventually.of_forall (fun σ hσ => h_bound hσ)
  ...
  have hfac :
    (fun σ =>
      (∫ t in I.interval, ((Zk.card : ℝ) * (∑ x ∈ Zk, (Ksigma σ (t - x))^2)) ∂(volume)) * σ)
    = (fun σ =>
        (Zk.card : ℝ) * (∫ t in I.interval, (∑ x ∈ Zk, (Ksigma σ (t - x))^2) ∂(volume)) * σ) := by
    funext σ; simp [mul_comm, MeasureTheory.integral_const_mul]
...
lemma integral_Ksigma_sq_mul_sigma_le_pi_over_two
  ...
  calc
    ...
    _ = Real.pi / 2 := by field_simp [hσ.ne']

lemma Vk_sq_pointwise_le_card_sum
    (Zk : Finset ℝ) (σ t : ℝ) :
    (Vk Zk σ t)^2 ≤ (Zk.card : ℝ) * ∑ γ ∈ Zk, (Ksigma σ (t - γ))^2 := by
  classical
...
lemma Vk_sq_integral_le_card_sum
    ...
    ∫ t in I.interval, (Vk Zk σ t)^2 ∂volume
      ≤ (Zk.card : ℝ) * ∑ γ ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - γ))^2 ∂volume := by
  classical
  have hAE :
      ∀ᵐ x ∂volume,
        x ∈ I.interval →
          (Vk Zk σ x)^2
            ≤ (Zk.card : ℝ) * ∑ γ ∈ Zk, (Ksigma σ (x - γ))^2 := by
    refine Filter.Eventually.of_forall ?_
    intro x hx
  ...
  have hInt₂ :
      IntegrableOn
        (fun t =>
          (Zk.card : ℝ) * ∑ γ ∈ Zk, (Ksigma σ (t - γ))^2)
        I.interval volume := by
    have hsum :
        Continuous fun t : ℝ =>
          ∑ γ ∈ Zk, (Ksigma σ (t - γ))^2 := by
      apply continuous_finset_sum
      intro γ _hγ
      unfold Ksigma
      have hden : Continuous fun t : ℝ => (t - γ) ^ 2 + σ ^ 2 :=
  ...
  have hmono :
      ∫ t in I.interval, (Vk Zk σ t)^2 ∂volume
        ≤ ∫ t in I.interval,
            (Zk.card : ℝ) * ∑ γ ∈ Zk, (Ksigma σ (t - γ))^2 ∂volume :=
  ...
  have hsum :
      ∫ t in I.interval, ∑ γ ∈ Zk, (Ksigma σ (t - γ))^2 ∂volume
        = ∑ γ ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - γ))^2 ∂volume := by
    classical
    simpa using
      (integral_finset_sum
  ...
  have hswap :
      (Zk.card : ℝ) *
          ∑ γ ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - γ))^2 ∂volume
        = ∫ t in I.interval,
            (Zk.card : ℝ) * ∑ γ ∈ Zk, (Ksigma σ (t - γ))^2 ∂volume := by
    simp [MeasureTheory.integral_const_mul]
    aesop
...
lemma diag_integral_mul_sigma_le_card_pi
    (I : WhitneyInterval) (Zk : Finset ℝ) {σ : ℝ} (hσ : 0 < σ) :
    (∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume) * σ
  ...
  have hσswap :
      σ * ∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume
        = ∑ γ ∈ Zk, σ * ∫ t in I.interval, (Ksigma σ (t - γ))^2 ∂volume := by
    classical
...
lemma Vk_sq_integral_mul_sigma_le_card_sq_pi
  ...
  have hσ_nonneg : 0 ≤ σ := hσ.le
  have hdiag :
      ((∑ γ ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - γ))^2 ∂volume) * σ)
        ≤ (Zk.card : ℝ) * (Real.pi / 2) := by
    have hswap :
        (∑ γ ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - γ))^2 ∂volume)
          = ∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume := by
      classical
      have hInt :
          ∀ γ ∈ Zk,
            Integrable (fun t => (Ksigma σ (t - γ))^2)
  ...
  calc
    ...
    _ = (Zk.card : ℝ) *
            ((∑ γ ∈ Zk, ∫ t in I.interval, (Ksigma σ (t - γ))^2 ∂volume) * σ) := by
          ring
...
lemma norm_diag_integral_mul_sigma_le_card_pi
    ...
    ‖(∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume) * σ‖
      ≤ (Zk.card : ℝ) * (Real.pi / 2) := by
  have h_nonneg :
      0 ≤ ∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume :=
  ...
  calc
    ‖(∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume) * σ‖
        = ‖∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume‖
            * ‖σ‖ := norm_mul _ _
    _ = (∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume) * σ := by
          simp [hσnorm]; exact Or.symm (Or.inr h_nonneg)
...
lemma integrand_diagonal_measurable_full (α : ℝ) (I : WhitneyInterval) (Zk : Finset ℝ) :
    AEStronglyMeasurable (fun σ => (∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume) * σ)
      (Measure.restrict volume (Set.Ioc (0 : ℝ) (α * I.len))) := by
  -- Composition of measurable functions
  have h1 : AEStronglyMeasurable (fun σ => ∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume)
      (Measure.restrict volume (Set.Ioc (0 : ℝ) (α * I.len))) := by
    by_cases h : 0 < α * I.len
    · have hI_bounded : Bornology.IsBounded I.interval := by
        rw [WhitneyInterval.interval]
...
theorem annularEnergyDiag_le
  ...
  have h_inner :
    ...
    have h_int1 :
      Integrable (fun t => (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2))
        (Measure.restrict volume I.interval) := by
    ...
    have h_int2 :
      Integrable (fun _ => (Zk.card : ℝ) * (σ^2 / r^4))
        (Measure.restrict volume I.interval) := by
      have hI_finite : volume I.interval < ⊤ := by
    ...
    have h_const_eval :
      ...
      have hvol_real :
        ...
        -- `(μ.restrict I).real univ = (volume I.interval).toReal = 2 * I.len`
        simp [Measure.real, hvolI, hv_len]
      -- Then evaluate the constant integral: `∫_I c = μ.real I * c`
  ...
  have hint_const :
    ...
    -- first: `Set.Ioc (0, α * I.len)` has finite Lebesgue measure
    have hfin : volume (Set.Ioc (0 : ℝ) (α * I.len)) < ⊤ := by
      simp [Real.volume_Ioc]
    -- hence the restricted measure is finite
    have hIsFin :
      IsFiniteMeasure (Measure.restrict volume (Set.Ioc (0 : ℝ) (α * I.len))) :=
  ...
  have hσ_int_mono :
    ...
    have hIntL :
      ...
      · apply MeasureTheory.HasFiniteIntegral.of_bounded (C := ((2 * I.len) * (Zk.card : ℝ) / r^4) * (α * I.len)^3)
        refine (ae_restrict_iff' measurableSet_Ioc).mpr ?_
        exact Filter.Eventually.of_forall (fun σ hσ => by
          rw [norm_mul, Real.norm_of_nonneg (le_of_lt hσ.1)]
          have h_int_nonneg : 0 ≤ ∫ t in I.interval, (∑ γ ∈ Zk, (Ksigma σ (t - γ))^2) ∂volume := by
            refine integral_nonneg_of_ae ?_
    ...
    have hIntConst :
      ...
      have hMass_toReal :
        ((Measure.restrict volume (Set.Ioc (0 : ℝ) (α * I.len))) Set.univ).toReal
          = α * I.len := by
        have hvol' :
            volume (Set.Ioc (0 : ℝ) (α * I.len))
              = ENNReal.ofReal (α * I.len) := by
          simp [Real.volume_Ioc]
        have :
            (Measure.restrict volume (Set.Ioc (0 : ℝ) (α * I.len))) Set.univ
  ...
  have hcompare :
    ...
    calc (2 * I.len) / r^4 * (α * I.len)^4
      ...
      _ ≤ (2 * I.len) * (α ^ 4 * (16 / 4 ^ k)) := by
          gcongr
      _ = (16 * α ^ 4) * (2 * I.len) / 4 ^ k := by field_simp
...
theorem annular_balayage_L2
  ...
  have hC0 : 0 ≤ (0 : ℝ) := by simp
  have hR : (0 : ℝ) * (2 * _I.len) / ((4 : ℝ) ^ k) * (_Zk.card : ℝ) = 0 := by simp
  have hbound : annularEnergy _α _I _Zk ≤
      (0 : ℝ) * (2 * _I.len) / ((4 : ℝ) ^ k) * (_Zk.card : ℝ) := by
