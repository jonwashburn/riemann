...
theorem completedRiemannZeta₀_finiteOrder :
    ComplexAnalysis.Hadamard.EntireOfFiniteOrder (3 / 2 : ℝ) completedRiemannZeta₀ := by
  constructor
...
theorem zeta_minus_pole_entire_finiteOrder :
    ∃ ρ : ℝ, ComplexAnalysis.Hadamard.EntireOfFiniteOrder ρ zetaTimesSMinusOne_entire := by
  classical
  refine ⟨(2 : ℝ), ?_⟩
  constructor
  · exact zetaTimesSMinusOne_entire_differentiable
...
/-! ## Hadamard factorization for Λ₀ (genus 1) -/

open ComplexAnalysis.Hadamard

theorem completedRiemannZeta₀_hadamard_factorization (hz : ZeroData completedRiemannZeta₀) :
    ∃ (P : Polynomial ℂ),
      P.degree ≤ 1 ∧
      ∀ z : ℂ,
        completedRiemannZeta₀ z =
          Complex.exp (Polynomial.eval z P) *
            z ^ hz.ord0 *
            ∏' n : ℕ, weierstrassFactor 1 (z / hz.zeros n) := by
  have hρ : (0 : ℝ) ≤ (3 / 2 : ℝ) := by norm_num
  have hf : EntireOfFiniteOrder (3 / 2 : ℝ) completedRiemannZeta₀ := by
    simpa using completedRiemannZeta₀_finiteOrder
  have hfloor : (⌊(3 / 2 : ℝ)⌋₊ : ℕ) = 1 := by
    -- `1 ≤ 3/2 < 2`
  ...
  -- Apply the general Hadamard factorization theorem and simplify `Nat.floor (3/2)`.
  simpa [hfloor] using
    (Complex.hadamard_factorization_main (ρ := (3 / 2 : ℝ)) (f := completedRiemannZeta₀) (hρ := hρ) hf hz)

theorem completedRiemannZeta₀_hadamard_factorization_natDegree (hz : ZeroData completedRiemannZeta₀) :
    ∃ (P : Polynomial ℂ),
      P.natDegree ≤ 1 ∧
      ∀ z : ℂ,
        completedRiemannZeta₀ z =
          Complex.exp (Polynomial.eval z P) *
            z ^ hz.ord0 *
            ∏' n : ℕ, weierstrassFactor 1 (z / hz.zeros n) := by
  rcases completedRiemannZeta₀_hadamard_factorization (hz := hz) with ⟨P, hdeg, hfac⟩
  refine ⟨P, ?_, hfac⟩
